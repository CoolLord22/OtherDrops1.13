...
public class CreatureDrop extends DropType {
    ...
    public CreatureDrop(IntRange amount, EntityType mob, Data mobData, double percent, CreatureDrop passenger, String displayName) { // Rome
    ...
    protected DropResult performDrop(Target source, Location where, DropFlags flags) {
        ...
        while (amount-- > 0) {
            dropResult.addWithoutOverride(dropCreatureWithRider(where, flags.recipient, type, data, this.getPassenger(), null, flags.getEvent(), flags.spawnReason));
        }
        ...
        if (displayName != null && !displayName.isEmpty()) {
            ...
            for (Entity ent : dropResult.getDropped()) {
                if (ent instanceof LivingEntity lEnt) {
                    String parsedCustomName = new ODVariables().setPlayerName(flags.getRecipientName()).parse(displayName);
    ...
    public static DropType parse(String drop, String state, IntRange amount, double chance) {
        ...
        // Check if Bukkit version is compatible with custom mob names (only in 1.5 and later) - if it's not then set to an empty string
        if (!displayName.isEmpty()) displayName = isVersionCompatibleWithCustomMobName(displayName);
        ...
        Log.logInfo("Parsing the creature drop... creature=" + creature + " data=" + data, EXTREME);
        return new CreatureDrop(amount, creature, data, chance, passenger, displayName);
    }

    private static String isVersionCompatibleWithCustomMobName(String displayName) {
        try {
            String versionString = Bukkit.getBukkitVersion().substring(0, 6).replaceAll("[.]", "");
            int bukkitVersion = Integer.parseInt(versionString);
            if (bukkitVersion >= 15) return displayName;
             else {
