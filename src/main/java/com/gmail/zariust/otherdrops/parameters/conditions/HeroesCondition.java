...
public class HeroesCondition extends Condition {
    ...
    static private final Map<String, Type> matches = new HashMap<>();
    ...
    protected boolean checkInstance(CustomDrop drop, OccurredEvent occurrence) {
        if (!Dependencies.hasHeroes()) return false; // shouldn't happen, but just in case
        ...
        return switch (type) {
            case CLASS ->
                    manager.getHero(occurrence.getPlayerAttacker()).getHeroClass().toString().equalsIgnoreCase(parameterValue);
            case LEVEL ->
                    String.valueOf(manager.getHero(occurrence.getPlayerAttacker()).getLevel()).equalsIgnoreCase(parameterValue);
        };
    }
    ...
    public List<Condition> parse(ConfigurationNode parseMe) {
        ...
        // parameters at all
        if (!Dependencies.hasHeroes()) return conditions;
