...
public class SoundAction extends Action {
    ...
    public SoundAction(Object object, SoundLocation damageEffectType2) {
        ...
        if (object instanceof List) {
            ...
            for (Object sub : stringList) {
                if (sub instanceof String) parseValue((String) sub);
                else if (sub instanceof Integer) parseValue(String.valueOf(sub));
            }
        } else if (object instanceof Map) {
            Log.dMsg("MAP detected");
            this.pickOne = true;
            @SuppressWarnings("unchecked") Map<Object, Object> stringList = (Map<Object, Object>) object;
    ...
    public boolean act(CustomDrop drop, OccurredEvent occurence) {
        if (sounds != null) {
            if (pickOne) {
                process(drop, occurence, sounds.get(OtherDrops.rng.nextInt(sounds.size())));
            } else {
    ...
    private void playSound(ODSound sound, Location location) {
        ...
        if (sound.volume != null) volume = sound.volume.getRandomIn(OtherDrops.rng);
        if (sound.pitch != null) pitch = sound.pitch.getRandomIn(OtherDrops.rng);

        Log.dMsg("Playing sound '" + sound.sound.toString() + "'/" + volume + "v/" + pitch + "p at location: " + location.toString());
        location.getWorld().playSound(location, sound.sound, volume.floatValue(), pitch.floatValue());
    }
    ...
    public List<Action> parse(ConfigurationNode parseMe) {
        ...
        for (String key : matches.keySet()) {
            if (parseMe.get(key) != null) actions.add(new SoundAction(parseMe.get(key), matches.get(key)));
        }
        return actions;
    }
}
