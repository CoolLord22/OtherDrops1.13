...
import main.java.com.gmail.zariust.otherdrops.data.entities.*;
...
public class CreatureData implements Data, RangeableData {
    ...
    // Map of EntityTypes to new class based creature data, for ease of lookup later on note: there should be only one
    // line per entity, or things could get messy
    static {
        ...
        // Scan through all entity types and if there's no current mapping then check if it's an Ageable or LivingEntity and assign a mapping
        for (EntityType type : EntityType.values()) {
            Class<?> typeClass = type.getEntityClass();
            if (typeClass != null) {
                ArrayList<Class<?>> data = new ArrayList<>();
                if (aMap.containsKey(type)) data = new ArrayList<>(aMap.get(type));
                if (Ageable.class.isAssignableFrom(typeClass)) data.add(AgeableData.class);
                if (LivingEntity.class.isAssignableFrom(typeClass)) data.add(LivingEntityData.class);
                if (Tameable.class.isAssignableFrom(typeClass)) data.add(TameableData.class);
                if (Steerable.class.isAssignableFrom(typeClass)) data.add(SteerableData.class);
                aMap.put(type, data);
            }
        }
    ...
    private String get(EntityType type) {
        Log.logInfo("CreatureData: get(" + type.toString() + "), shouldn't be here (should be in specific mob data) - please let developer know.", Verbosity.HIGH);
        return "";
    }
    ...
    public void setOn(Entity mob, Player owner) {
        ...
        Log.logInfo("CreatureData: setOn(" + mob.toString() + ", " + owner.toString() + "), shouldn't be here (should be in specific mob data) - please let developer know.", Verbosity.HIGH);
    }
    ...
    public static CreatureData parseFromString(String state) {
        Log.logInfo("CreatureData: parseFromString(" + state + "), shouldn't be here (should be in specific mob data) - please let developer know.", Verbosity.HIGH);
        return null;
    }
