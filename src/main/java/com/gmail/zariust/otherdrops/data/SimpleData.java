...
public class SimpleData implements Data, RangeableData {
    ...
    private String get(Material mat) {
        ...
        try {
            ...
            Log.logWarning("SimpleData.get - nullpointer exception for material: " + mat, Verbosity.HIGH);
    ...
    public static Data parse(Material mat, String state) throws IllegalArgumentException {
        ...
        if (state.matches("[0-9][0-9~.-]+[0-9]") || state.startsWith("RANGE")) return RangeData.parse(state);
        ...
        switch (mat) {
            ...
            case CAKE:
                ...
                if (state.startsWith("EATEN-")) cake.setSlicesEaten(Integer.parseInt(state.substring(6)));
                else if (state.startsWith("LEFT-")) cake.setSlicesRemaining(Integer.parseInt(state.substring(5)));
