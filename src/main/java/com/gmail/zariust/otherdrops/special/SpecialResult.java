...
public abstract class SpecialResult {
    ...
    private final SpecialResultHandler handler;
    ...
    /**
     * Mark an argument as having been parsed and used by the event. This does several useful things:
     * <ul>
    ...
    public static List<SpecialResult> parseFrom(ConfigurationNode node) {
        ...
        // things
        List<SpecialResult> result = new LinkedList<>();
        for (String eventCall : events) {
            ...
            if (split.length > 1) {
                ...
                if (!event.arguments.isEmpty()) {
                    Log.logWarning("While parsing arguments for event " + event.getTag() + ", the " + "following invalid arguments were ignored: " + event.arguments.toString());
                }
